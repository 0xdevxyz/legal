'use client';

import { useState } from 'react';
import { useRouter } from 'next/navigation';
import { firebaseAuth, isFirebaseEnabled } from '@/lib/firebase';

const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'https://api.complyo.tech';

interface SocialLoginButtonsProps {
  plan?: string;
  mode?: 'login' | 'register';
}

export default function SocialLoginButtons({ plan = 'ki', mode = 'login' }: SocialLoginButtonsProps) {
  const router = useRouter();
  const [loading, setLoading] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  /**
   * Firebase Google Sign-In
   */
  const handleGoogleFirebase = async () => {
    if (!isFirebaseEnabled) {
      setError('Firebase Auth ist nicht konfiguriert');
      return;
    }

    setLoading('google');
    setError(null);

    try {
      // Sign in with Google (Firebase Popup)
      const result = await firebaseAuth.signInWithGoogle();
      
      // Get Firebase ID Token
      const idToken = await result.user.getIdToken();

      // Verify with Complyo Backend
      const response = await fetch(`${API_BASE}/api/auth/firebase-verify`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id_token: idToken, plan })
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.detail || 'Firebase Authentifizierung fehlgeschlagen');
      }

      const data = await response.json();

      // Store Complyo tokens
      localStorage.setItem('access_token', data.access_token);
      localStorage.setItem('refresh_token', data.refresh_token);
      localStorage.setItem('user', JSON.stringify(data.user));

      // Redirect to dashboard or payment
      if (mode === 'register') {
        // Redirect to Stripe Checkout
        const checkoutResponse = await fetch(`${API_BASE}/api/payment/create-checkout`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${data.access_token}`,
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({ plan_type: plan })
        });

        if (checkoutResponse.ok) {
          const { checkout_url } = await checkoutResponse.json();
          window.location.href = checkout_url;
        } else {
          throw new Error('Fehler beim Erstellen der Checkout-Session');
        }
      } else {
        router.push('/dashboard');
      }

    } catch (err: any) {
      console.error('Google Firebase Error:', err);
      setError(err.message || 'Google Anmeldung fehlgeschlagen');
    } finally {
      setLoading(null);
    }
  };

  /**
   * Fallback: OAuth via Backend (wenn Firebase nicht konfiguriert ist)
   */
  const handleGoogleOAuth = async () => {
    setLoading('google');
    setError(null);

    try {
      const response = await fetch(`${API_BASE}/api/auth/google`);
      const { auth_url } = await response.json();
      
      // Redirect to Google OAuth
      window.location.href = auth_url;
    } catch (err: any) {
      console.error('Google OAuth Error:', err);
      setError('Google Anmeldung fehlgeschlagen');
      setLoading(null);
    }
  };

  const handleGoogleClick = () => {
    if (isFirebaseEnabled) {
      handleGoogleFirebase();
    } else {
      handleGoogleOAuth();
    }
  };

  // Don't render if both Firebase and OAuth are unavailable
  if (!isFirebaseEnabled) {
    return null;
  }

  return (
    <div className="space-y-3">
      {/* Error Message */}
      {error && (
        <div className="bg-red-500 bg-opacity-10 border border-red-500 text-red-500 px-4 py-2 rounded text-sm">
          {error}
        </div>
      )}

      {/* Google Sign-In Button */}
      <button
        onClick={handleGoogleClick}
        disabled={loading === 'google'}
        className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white text-gray-900 rounded-lg font-semibold hover:bg-gray-100 transition disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {loading === 'google' ? (
          <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-gray-900"></div>
        ) : (
          <>
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Mit Google {mode === 'register' ? 'registrieren' : 'anmelden'}
          </>
        )}
      </button>

      {/* Divider */}
      <div className="relative flex items-center">
        <div className="flex-grow border-t border-gray-700"></div>
        <span className="flex-shrink mx-4 text-gray-500 text-sm">oder</span>
        <div className="flex-grow border-t border-gray-700"></div>
      </div>

      {/* Note about Firebase */}
      {isFirebaseEnabled && (
        <p className="text-xs text-gray-500 text-center">
          ✅ Firebase Auth aktiv
        </p>
      )}
    </div>
  );
}
