...
- **Global scope**: `/home/clawd/saas/legal/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/clawd/saas/legal/.claude/notes/`)
...
2. Glob `/home/clawd/saas/legal/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/clawd/saas/legal/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/clawd/saas/legal/.claude/notes/` — works in any directory
