...
def confidence_to_float(confidence_str: str) -> float:
    """Konvertiert Confidence-String zu Float"""
    mapping = {
        'high': 0.9,
        'medium': 0.7,
        'low': 0.5
...
async def classify_new_updates():
    ...
    for update in unclassified:
        ...
        try:
            ...
            # In DB speichern (einzelne Spalten statt JSON!)
            await conn.execute('''
            ...
            ''', 
                ...
                confidence_to_float(classification.confidence.value),  # ✅ Convert to float
                classification.impact_score,
                classification.primary_action.action_type.value,
                classification.primary_action.button_text,
                classification.primary_action.button_color,
                classification.primary_action.icon,
                classification.primary_action.priority,
                classification.primary_action.title,
                classification.primary_action.description,
                classification.primary_action.estimated_time,
                classification.primary_action.requires_paid_plan,
